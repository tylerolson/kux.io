...
function init() {
  ...
  offsetX = Math.floor(-(canvas.width / 2) / tileSize) * tileSize;
  offsetY = Math.floor(-(canvas.height / 2) / tileSize) * tileSize;
}
...
function drawMap() {
  for (row = 0; row < mapSize; row++) {
    for (col = 0; col < mapSize; col++) {
      x = (tileSize * row) + canvas.width / 2;
      y = (tileSize * col) + canvas.height / 2;
      if (mapArea[[row, col]] == "*") {
