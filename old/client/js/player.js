...
function Player(color, name, local) {
	...
	this.getSqaureX = function() {
		return -Math.round(offsetX / TILE_SIZE);
	}
	this.getSqaureY = function() {
		return -Math.round(offsetY / TILE_SIZE);
	}

	this.update = function() {
		if (offsetX % TILE_SIZE === 0 && offsetY % TILE_SIZE === 0) { //if offsetx is in a sqaure
			this.inSqaure = true;
		} else
			this.inSqaure = false;
...
function checkPlayerInSqaure(playerComponent) {
	return playerComponent.inSqaure;
}
